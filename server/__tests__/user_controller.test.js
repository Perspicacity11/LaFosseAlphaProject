...
describe('User Controller', () => {
  ...
  describe('update', () => {
    it('should update a user and return the result with status 200', async () => {
      req.params = { id: '5' };
      req.body = { username: 'updated', total_score: 50 };

      const mockUser = {
        update: jest.fn().mockResolvedValue({ id: 5, username: 'updated', total_score: 100 })
      };
      ...
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({ id: 5, username: 'updated', total_score: 100 });
    });
